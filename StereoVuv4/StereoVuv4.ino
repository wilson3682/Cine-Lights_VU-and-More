...
void ripple3() {
  ...
  n = analogRead(A5);             // Raw reading from mic
  ...
  if(peak > 0 && peak <= NUM_LEDS-1) strip1.setPixelColor(peak,Wheel(map(peak,0,strip1.numPixels()-1,30,150)));
   ...
   n = analogRead(A4);             // Raw reading from mic
  n = abs(n - 512 - DC_OFFSET);        // Center on zero
  n = (n <= NOISE) ? 0 : (n - NOISE);  // Remove noise/hum
  lvl = ((lvl * 7) + n) >> 3;    // "Dampened" reading (else looks twitchy)
...
// Every few frames, make the peak pixel drop by 1:
  ...
  for(i=0; i<NUM_LEDS_HALF; i++) {
    if(i >= height) {              
      strip1.setPixelColor(NUM_LEDS_HALF-i-1,   0,   0, 0);
      strip1.setPixelColor(NUM_LEDS_HALF+i,   0,   0, 0);
    }
    else {
