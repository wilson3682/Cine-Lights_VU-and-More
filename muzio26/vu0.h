#ifndef __INC_MUZIO_VU_H
#define __INC_MUZIO_VU_H
#include <Arduino.h>
#include "common.h"
#include "util.h"


void vu_normal_center_out() // vu
{
  uint8_t i;
  uint16_t minLvlLeft, maxLvlLeft;
  int height;

  int n = readMic(&height);
  ...
  for (i = 0; i < N_PIXELS; i++)
  {
    if (i >= height)
      ...
      ledsLeft[i] = Wheel2(map(i, 0, N_PIXELS - 1, 30, 150));
  }
  // Draw peak dot
