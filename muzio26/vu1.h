#ifndef __INC_MUZIO_VU_1_H
#define __INC_MUZIO_VU_1_H
#include <Arduino.h>
#include "common.h"
#include "util.h"

void vu_rainbow_normal() // vu1
{
  uint8_t i;
  ...
  {
    CRGB color = i >= height ? CRGB::Black : Wheel2(map(i, 0, N_PIXELS_HALF - 1, 30, 150));
  ...
  {
    CRGB color = Wheel2(map(peak, 0, N_PIXELS_HALF - 1, 30, 150));
    ledsLeft[N_PIXELS_HALF - peakLeft - 1] = color; 
    ledsLeft[N_PIXELS_HALF + peakLeft] = color;
  }

  FastLED.show(); // Update strip

  // Every few frames, make the peak pixel drop by 1:
  if (++dotCountLeft >= PEAK_FALL3) // fall rate
  { 
    if (peakLeft > 0)
      peakLeft--;
    dotCountLeft = 0;
  ...
  if (peakLeft > 0 && peakLeft <= N_PIXELS_HALF - 1)
  {
    CRGB color = Wheel2(map(peakLeft, 0, N_PIXELS_HALF - 1, 30, 150));
    ledsLeft[N_PIXELS_HALF - peakLeft - 1] = color; 
    ledsLeft[N_PIXELS_HALF + peakLeft] = color;
  }
