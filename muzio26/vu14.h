#ifndef __INC_MUZIO_VU_14_H
#define __INC_MUZIO_VU_14_H
#include <Arduino.h>
#include "common.h"
#include "util.h"

void vu_white_red_dots()
{
    ...
    {
        ledsLeft[i] = i >= height ? CRGB::Black : color;
    }

    // Draw peak dot
    if (peakLeft > 0 && peakLeft <= N_PIXELS - 1)
        ledsLeft[peakLeft] = CRGB(map(peakLeft, 0, N_PIXELS - 1, red, red));
